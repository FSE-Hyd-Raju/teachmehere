    ...
    return (
      category && (
        <View style={{ marginBottom: 10 }}>
          {!!category.subCategories && !!category.subCategories.length && <Text style={{ padding: 10, paddingTop: 0, fontSize: 15, letterSpacing: 1, color: "black", marginTop: 10 }}>
            Sub Categories
          </Text>
          }
          <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} style={{ marginTop: 5 }}>
            {!!category.subCategories && !!category.subCategories.length &&
              category.subCategories.map((item, index) => {
                return (
                  <Chip
                    style={{ color: "black", margin: 5, padding: 3, borderColor: "lightgrey", borderWidth: 0.3, backgroundColor: "rgba(243, 246, 252, 0.7)" }}
      ...
      <View >
        {!!categorySkills && !!categorySkills.length &&
          categorySkills.map(skill => {
            return (
              <CourseCard course={skill} courseClicked={() => showDetails(skill)} />
            )
          })}
